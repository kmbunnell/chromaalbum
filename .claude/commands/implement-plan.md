...
2. **Work through Implementation Steps in order.** For each step:
   ...
   d. Run the relevant Gradle command:
      - Unit tests: `./gradlew :app:testDebugUnitTest --tests "*.ClassName"`
      - Build check: `./gradlew :app:assembleDebug` (when no targeted test exists)

3. **After all steps:**
   a. `./gradlew :app:spotlessApply` then `./gradlew :app:spotlessCheck` — fix remaining violations.
   b. `./gradlew :app:lint` — fix new warnings.
   c. `./gradlew :app:testDebugUnitTest` (full suite). If a previously-passing test is now failing, list it as a **regression** and stop — do not fix it. Let the user decide.
   d. Tell the user:
